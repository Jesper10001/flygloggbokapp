...
export default function DroneOnboardingScreen() {
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {step === 0 ? (
        <View style={styles.step}>
          <View style={styles.iconWrap}>
            <Ionicons name="hardware-chip" size={56} color={Colors.primary} />
          </View>
          <Text style={styles.title}>{t('drone_onb_1_title')}</Text>
          <Text style={styles.body}>{t('drone_onb_1_body')}</Text>
          <View style={styles.bullets}>
            <Bullet text={t('drone_onb_bullet_models')} />
            <Bullet text={t('drone_onb_bullet_batteries')} />
            <Bullet text={t('drone_onb_bullet_certs')} />
          </View>
        </View>
      ) : (
        <View style={styles.step}>
          ...
          <Text style={styles.title}>{t('drone_onb_2_title')}</Text>
          <Text style={styles.body}>{t('drone_onb_2_body')}</Text>
          <View style={styles.bullets}>
            <Bullet text={t('drone_onb_bullet_fast')} />
            <Bullet text={t('drone_onb_bullet_auto_cycles')} />
            <Bullet text={t('drone_onb_bullet_export')} />
          </View>
        </View>
      )}
      ...
      <View style={{ gap: 10, marginTop: 24 }}>
        {step === 0 ? (
          ...
          <>
            <TouchableOpacity style={styles.primaryBtn} onPress={goAddDrone} activeOpacity={0.85}>
              <Ionicons name="hardware-chip-outline" size={16} color={Colors.textInverse} />
              <Text style={styles.primaryBtnText}>{t('drone_onb_add_first')}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.secondaryBtn} onPress={finish} activeOpacity={0.7}>
              <Text style={styles.secondaryBtnText}>{t('drone_onb_skip')}</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
