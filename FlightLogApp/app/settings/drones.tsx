...
function DroneFormModal({
  ...
  const handleScan = async (fromCamera: boolean) => {
    // Token-styrt, inte premium-låst: fri nivå får skanna tills engångspotten tar slut.
    if (!hasTokenQuota()) {
      if (isPremium || isMax) { showMonthlyTokenLimitAlert(); } else { Alert.alert(t('premium_modal_title'), t('premium_modal_desc')); }
      return;
    }
    try {
      let result;
  ...
  const save = async () => {
    const data = {
      ...
      category,
      drone_class: initial?.drone_class ?? '', // klass sätts i drönar-modalen (Log Flight)
      notes: notes.trim(),
    };
