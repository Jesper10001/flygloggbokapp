...
export default function DashboardScreen() {
  ...
  const greeting = useMemo(() => {
    ...
    // Happy New Year: 1 jan 00:00 - 2 jan 23:59
    if ((m === 0 && d === 1) || (m === 0 && d === 2)) {
      return t('happy_new_year');
    }

    const h = now.getHours();
    const name = h < 4 ? '' : profileName || '';
    const g = h < 4 ? t('early_bird') : h < 12 ? t('good_morning') : h < 18 ? t('good_afternoon') : t('good_evening');
