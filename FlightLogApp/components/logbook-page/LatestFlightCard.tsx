// "LATEST FLIGHT"-hero: NightRoute + block-tid + natt-caption + stadspar + 5-cellsremsa.
    ...
    <View style={{ paddingHorizontal: 14, paddingTop: 12, paddingBottom: 4 }}>
      <TouchableOpacity onPress={onPress} activeOpacity={0.85} style={{ borderRadius: 16, overflow: 'hidden', borderWidth: 1, borderColor: accent + '55' }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingBottom: 6 }}>
          <NightRoute dep={f.dep_place} arr={f.arr_place} nightFrac={nightFrac} size={30} accent={accent} dayColor={Colors.textPrimary} />
          <View style={{ flex: 1 }} />
          <View style={{ alignItems: 'flex-end' }}>
            <Text style={{ fontFamily: FONT_MONO, fontSize: 17, fontWeight: '700', color: Colors.textPrimary }}>{formatTime(f.total_time)}</Text>
            <Text style={{ fontFamily: FONT_MONO, fontSize: 9, color: Colors.textMuted }}>BLOCK</Text>
          </View>
        </View>
        {/* natt-caption */}
        {f.night > 0 && (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 14, paddingBottom: 8 }}>
            ...
            {darkOnsetUtc ? <Text style={{ fontFamily: FONT_MONO, fontSize: 9.5, color: Colors.textMuted }}>· dark {darkOnsetUtc}z</Text> : null}
          </View>
        )}
        <Text style={{ fontSize: 11.5, color: Colors.textSecondary, paddingHorizontal: 14, paddingBottom: 10 }}>{city(f.dep_place)} → {city(f.arr_place)}</Text>
        {/* 5-cellsremsa */}
        <View style={{ flexDirection: 'row', borderTopWidth: 1, borderTopColor: accent + '33' }}>
          {strip.map(([k, v, hot], i) => (
            <View key={k} style={{ flex: 1, paddingVertical: 8, paddingHorizontal: 6, borderLeftWidth: i ? 1 : 0, borderLeftColor: accent + '22', alignItems: 'center' }}>
              <Text style={{ fontFamily: FONT_MONO, fontSize: 7.5, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase', color: Colors.textMuted }}>{k}</Text>
