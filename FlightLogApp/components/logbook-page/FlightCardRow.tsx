...
export function FlightCardRow({ flight: f, accent, onPress }: { flight: Flight; accent: string; onPress: () => void }) {
  const { formatTime } = useTimeFormat();
  const role = roleLabel(f);
  const roleColor = isPicFlight(f) ? accent : Colors.textSecondary;
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.7}
      style={{ flexDirection: 'row', alignItems: 'center', gap: 13, paddingVertical: 12, paddingHorizontal: 14, borderTopWidth: 1, borderTopColor: Colors.separator }}>
      {/* datumchip */}
