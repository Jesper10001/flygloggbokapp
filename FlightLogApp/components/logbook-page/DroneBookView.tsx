// Drönar-Book-vy (in-flik) — KLON av manned BookView, samma motor och layout, men
// datakällan är drönarflygningar (droneToFlightRow → Flight[]) och böckerna lagras
// med kind='drone'. Två stående sidor i full bredd; bläddra/fullskärm under höger
// sida; Modify / New logbook via samma BookSetupSheet (appMode='drone').
...
import { listDigitalBooks, setActiveDigitalBook, type DigitalBook } from '../../db/digitalBooks';
...
export function DroneBookView({ accent, headerRight }: { accent: string; headerRight?: React.ReactNode }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setBooks(await listDigitalBooks('drone'));
      setLoading(false);
    })();
  }, []);

  // Ladda om böckerna vid fokus (t.ex. efter att man skapat en bok i helskärm /drone-logbook).
  useFocusEffect(useCallback(() => {
    listDigitalBooks('drone').then(setBooks);
    loadDroneMetaById().then(setDronesById).catch(() => {});
  }, []));

  // Drönarflygningar → mall-rader (Flight-form) EN gång; matas till motorn som manned.
  ...
  const slice = useMemo<BookSlice | null>(() => (book ? assignFlightsToBooks(books, flightRows).find((s) => s.book.id === book.id) ?? null : null), [book, books, flightRows]);
  const template = useMemo(() => (book ? getTemplate(book.template_id) : null), [book]);
  const spreads = useMemo<LogbookSpread[]>(() => {
    ...
    return buildBookSpreads(slice?.flights ?? [], template, {
      ...
      openingBalance: resolveOpeningBalance(book, books, flightRows, template),
      leadingEmptyRows: slice?.leadingEmptyRows ?? 0,
    });
  }, [book, template, slice, flightRows, books]);
  ...
  const ctrlBtn = { width: 44, height: 44, borderRadius: 22, backgroundColor: DR.surface, borderWidth: 1, borderColor: DR.border, alignItems: 'center', justifyContent: 'center' } as const;
  const secBtn = { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 12, borderRadius: 12, borderWidth: 1, borderColor: DR.border, backgroundColor: DR.surface } as const;
  ...
  const Header = (
    <View style={{ paddingTop: 6, paddingHorizontal: 14, paddingBottom: 4, flexDirection: 'row', alignItems: 'center', gap: 10, zIndex: 30 }}>
      <TouchableOpacity onPress={() => books.length > 1 && setShowBooks((v) => !v)} disabled={books.length <= 1}
        ...
        <Text numberOfLines={1} style={{ fontFamily: FONT_SERIF, fontSize: 26, fontWeight: '600', color: DR.text, flexShrink: 1 }}>{book?.name || 'Book'}</Text>
  ...
  if (loading) return <View style={{ flex: 1, backgroundColor: DR.background }}><ActivityIndicator color={accent} style={{ marginTop: 60 }} /></View>;
    ...
    return (
      <View style={{ flex: 1, backgroundColor: DR.background }}>
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 30, gap: 14 }}>
          ...
          <Text style={{ fontFamily: FONT_SERIF, fontSize: 20, fontWeight: '600', color: DR.text, textAlign: 'center' }}>No logbook yet</Text>
          <Text style={{ fontSize: 13, color: DR.text2, textAlign: 'center', lineHeight: 19 }}>Create a digital logbook to see your drone flights laid out as a real spread.</Text>
          <TouchableOpacity onPress={() => router.push('/drone-logbook')} activeOpacity={0.85}
            style={{ marginTop: 6, paddingHorizontal: 22, paddingVertical: 13, borderRadius: 13, backgroundColor: accent }}>
            <Text style={{ fontSize: 14, fontWeight: '700', color: DR.inkOnAccent }}>Create logbook</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
    ...
    <View style={{ flex: 1, backgroundColor: DR.background }}>
      ...
      <View style={{ paddingBottom: 4, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', zIndex: 30 }}>
        <TouchableOpacity onPress={() => spreads.length > 1 && setShowOverview((v) => !v)} disabled={spreads.length <= 1}
          ...
          <Text style={{ fontFamily: FONT_SERIF, fontSize: 16, fontWeight: '600', color: DR.text2 }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingTop: 2, paddingBottom: 28 }} showsVerticalScrollIndicator={false}>
        {current ? (
          <>
            ...
            <View style={{ width: '100%', height: width * ratioL, backgroundColor: DR.background }}>
              <SpreadWebView spread={current} template={template} pilotName={pilotName} timeFormat={timeFormat}
                width={width} signature={signature} interactive={false} bare bgColor={DR.background} side="left" margin={0}
                onAspect={(r) => { if (r > 0 && Math.abs(r - ratioL) > 0.01) setRatioL(r); }} />
            </View>
            {/* höger sida (under) — full bredd */}
            <View style={{ width: '100%', height: width * ratioR, backgroundColor: DR.background, marginTop: 6 }}>
              <SpreadWebView spread={current} template={template} pilotName={pilotName} timeFormat={timeFormat}
                width={width} signature={signature} interactive={false} bare bgColor={DR.background} side="right" margin={0}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 18, paddingVertical: 16 }}>
              <TouchableOpacity disabled={safeIdx <= 0} onPress={() => setIdx((i) => Math.max(0, i - 1))} activeOpacity={0.7}
                ...
                <Ionicons name="chevron-back" size={20} color={DR.text2} />
              </TouchableOpacity>
              <TouchableOpacity onPress={() => router.push('/drone-logbook?recent=1')} activeOpacity={0.7} style={ctrlBtn}>
              ...
              <TouchableOpacity disabled={safeIdx >= spreads.length - 1} onPress={() => setIdx((i) => Math.min(spreads.length - 1, i + 1))} activeOpacity={0.7}
                ...
                <Ionicons name="chevron-forward" size={20} color={DR.text2} />
              </TouchableOpacity>
            </View>
          ...
          <Text style={{ color: DR.muted, textAlign: 'center', marginTop: 40 }}>No pages yet</Text>
        )}
      </ScrollView>
      ...
      {showOverview && (
        <>
          ...
          <View style={{ position: 'absolute', top: 80, left: 0, right: 0, alignItems: 'center', zIndex: 50 }} pointerEvents="box-none">
            <View style={{ width: 250, maxHeight: 320, backgroundColor: DR.surface, borderWidth: 1, borderColor: DR.border, borderRadius: 14, overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.4, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 10 }}>
              <ScrollView showsVerticalScrollIndicator={false}>
                {spreads.map((sp, i) => (
                  <TouchableOpacity key={sp.spread_number} onPress={() => { setIdx(i); setShowOverview(false); }} activeOpacity={0.7}
                    style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 11, paddingHorizontal: 14, borderTopWidth: i ? 1 : 0, borderTopColor: DR.separator, backgroundColor: i === safeIdx ? accent + '1F' : 'transparent' }}>
                    <Text style={{ fontFamily: FONT_MONO, fontSize: 12, fontWeight: '700', color: i === safeIdx ? accent : DR.text }}>pp {sp.page_left}–{sp.page_right}</Text>
                    <View style={{ flex: 1 }} />
                    <Text style={{ fontFamily: FONT_MONO, fontSize: 10, color: DR.muted }}>{i + 1}/{spreads.length}</Text>
      ...
      {showBooks && (
        <>
          <TouchableOpacity activeOpacity={1} onPress={() => setShowBooks(false)}
            style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 40 }} />
          <View style={{ position: 'absolute', top: 44, left: 14, width: 250, maxHeight: 320, backgroundColor: DR.surface, borderWidth: 1, borderColor: DR.border, borderRadius: 14, overflow: 'hidden', zIndex: 60, shadowColor: '#000', shadowOpacity: 0.4, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 10 }}>
            <ScrollView showsVerticalScrollIndicator={false}>
              {[...books].sort((a, b) => (b.display_order - a.display_order) || (b.id - a.id)).map((b, i) => {
                ...
                return (
                  <TouchableOpacity key={b.id} activeOpacity={0.7}
                    onPress={async () => { setShowBooks(false); await setActiveDigitalBook(b.id, 'drone'); setBooks(await listDigitalBooks('drone')); }}
                    style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 12, paddingHorizontal: 14, borderTopWidth: i ? 1 : 0, borderTopColor: DR.separator, backgroundColor: active ? accent + '1F' : 'transparent' }}>
                    <Ionicons name="book-outline" size={15} color={active ? accent : DR.muted} />
                    <Text numberOfLines={1} style={{ flex: 1, fontFamily: FONT_SERIF, fontSize: 14, fontWeight: '600', color: active ? accent : DR.text }}>{b.name}</Text>
      ...
      <Modal visible={!!setup} animationType="slide" transparent supportedOrientations={['portrait']} onRequestClose={() => setSetup(null)}>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          {setup && (
            <BookSetupSheet mode={setup.mode} appMode="drone" initial={setup.initial} flights={flightRows} carryOpeningBalance={setup.carry} timeFormat={timeFormat}
              onClose={() => setSetup(null)} onSaved={() => { setSetup(null); listDigitalBooks('drone').then(setBooks); }} />
          )}
        </View>
      </Modal>
    </View>
